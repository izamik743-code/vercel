...
export class TelegramGiftsService {
  ...
  async openCase(caseId: string, userId: string): Promise<TelegramGift> {
    ...
    const { data: caseData } = await supabase.from("cases").select("price_ton").eq("id", caseId).single()
  ...
  async getOnlineStats(): Promise<OnlineStats> {
    ...
    const { data: statsData } = await supabase.from("online_stats").select("online_count").single()
  ...
  async updateOnlineCount() {
    ...
    await supabase.from("online_stats").update({ online_count: newCount }).eq("id", 1)
  }
  ...
  async addFakeWin() {
    ...
    // Get random gift
    const { data: gifts } = await supabase.from("telegram_gifts").select("id").order("RANDOM()").limit(1)
    ...
    if (gifts?.[0]) {
      ...
      const { data: oldWins } = await supabase
        .from("recent_wins")
        .select("id")
        .order("created_at", { ascending: false })
        .range(20, 100)
